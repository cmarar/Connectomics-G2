...
To process the data and run the prediction algortihm, we will need computers for each of our programmers.

For the mouse portion of the study, we will need an animal MRI machine, a mouse housing facility, a laboratory to run experiments, 

For the human portion of the study, we will need an MRI machine, an operating room, a recovery room, and a laboratory to run cognitive tests.

\section{Data Collection}
For the purposes of this project, we wish to divide all experimentation into three segments. 

\subsection{fMRI and DTI Data Collection}

\subsection{Deep Brain Stimulation}
The fMRI and DTI data will be used to generate brain connectomes for all patients. We wish to carry out this patient-directed treatment because the rate and progression of Alzheimer's disease differs greatly between patients. 
We hope to apply an algorithm to the connectomes that will provide us with information regarding the rate at which each affected brain area is changing, and which new areas are most likely to be affected. The purpose of this is to determine which brain regions the electrodes should be placed. The idea is that DBS will yield positive results if electrodes are placed in the regions that are changing faster.
